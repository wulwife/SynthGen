...
class Synthetic_waveforms:
    ...
    def _get_sensitivity(self):
        ...
        self.avg_sens = num.mean(avg_sens) if avg_sens else None
    ...
    def __extract_real_noise(self, evid, time_window, buffer, resampling_freq, highcut_freq, sds_root='/media/francesco/LaCie/COSEISMIQ', network='[2O][CN]'):
        ...
        st = client.get_waveforms(network, "*", "*", "[BHE]H[ENZ]", tini-buffer, tini+time_window+buffer, sds_type='D') #da cambiare
        st.detrend('demean')
    ...
    def __extract_global_noise(self, tini_glob, tend_glob, noise_win, resampling_freq, highcut_freq, sds_root='/media/francesco/LaCie/COSEISMIQ', network='[2O][CN]'):
        from obspy.clients.filesystem.sds import Client
        client=Client(sds_root, sds_type='D', format='MSEED', fileborder_seconds=30, fileborder_samples=5000)
        real_noise={}
        tini=UTCDateTime(tini_glob)
        tend=UTCDateTime(tend_glob)
        st = client.get_waveforms(network, "*", "*", "[BHE]H[ENZ]", tini, tend, sds_type='D') #da cambiare
        for tr in st:
            tr.trim(tini,tini+3*noise_win)
            tr.detrend('demean')
            tr.merge(method=0,fill_value=0)
    ...
    def _model_noise_continuum(self, starttime, endtime, time_window, buffer, resampling_freq, highcut_freq): #non c
        noise={}
        real_noise=self.__extract_global_noise(starttime, endtime, time_window, buffer, resampling_freq, highcut_freq)
        tini=UTCDateTime(starttime)
        tend=UTCDateTime(endtime)
        twin=tend-tini
        dt=1/resampling_freq
        df=1/twin
        nf=((resampling_freq/2)/df)
        fax=num.arange(nf)*df
        print(fax)
    ...
    def _model_noise_global(self, evid, time_window, buffer, resampling_freq, highcut_freq):
        noise={}
    ...
    def generate_waveforms_global(self, events, starttime, endtime, time_window, resampling_freq, highcut_freq, source_type='dc'):
        self.__get_sensitivity()
        self.__extract_global_noise(starttime, endtime, time_window, resampling_freq, highcut_freq)
        for evid in events.keys():
            event_dir = os.path.join(self.waveform_dir, evid)
            if not os.path.isdir(event_dir):
                os.mkdir(event_dir)
            tor=events[evid][0]
            source=self.__gen_source(events[evid], source_type)
            noise=self._model_noise_global(evid, time_window, buffer, resampling_freq, highcut_freq)
            self.__waveform_gen(tor, time_window, source, noise, event_dir)
